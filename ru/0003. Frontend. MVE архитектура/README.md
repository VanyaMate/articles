...
export const Posts: FC = memo(function Posts (props) {
...
Но, а теперь давайте подумаем. Вот мы, получается, используем какие-то проценты от всего что есть в Redux ради этого.
Зачем? Добавляем лишние 15 кб (в gzip) к бандлу. Слишком много. Не подходит.
...
1. ["Социальная сеть" (github)](https://github.com/VanyaMate/product).
   Там и сообщения реализованы и посты и друзья и облачное хранилище и всякое другое, но суть в том, что в проекте
   используется **MVA** и всё легко разрабатывается. То есть до этого я что-то разрабатывал, разрабатывал и в какой-то
   момент начинались сложности, а тут всё идет так как будто это новый проект, хотя он уже достаточно большой.

Вы можете посмотреть сами как это можно реализовать и что это такое.

Модели лежат в /src/app/model
Вьюшки в /src/[type]
Действия в /src/app/action

Ps. Я понимаю, что там могут в моделях, по подпискам, быть одинаковые функции, что можно это сделать лучше, но это уже
детали реализации.
...
Я искренне считаю, что это очень крутая архитектура получилась. Очень простая.

Для её реализации нам не нужны какие-то громоздкие инструменты, она регламентирует то как должно быть устроено
приложение.

Вам никто не мешает использовать этот подход вместе с другими.

Так же я советую, если вы не читали [прошлую статью](https://habr.com/ru/articles/824842/), прочесть и её, думаю в таком
случае будет больше понимания, что я хочу донести.

И я бы не писал вторую статью на эту тему если бы не считал, что это очень крутой подход. Просто мне кажется, что
прошлую статью не очень поняли и это, думаю, последняя попытка.
