...
Давайте представим такой простой сценарий.
Допустим у нас есть:
 ...
 * мы будем использовать createEffect. 
 ...
 * 
 * Таким образом можно подписываться на множество разных экшенов и 
 * использовать разные состояния (done, fail, finally, ...)
 */
...
Давайте создадим форму создания
...
Теперь давайте представим как этот компонент будет выглядеть просто при рендере.

- Сверху будет инпут и кнопка (форма создания новой задачи)
...
но это не важно. Главное просто о вашем потоке данных и представлять его в голове, а с таким подходом это очень легко.
...
Я его переделывал, но лишь частично.

Там всё переделывалось на extraReducers и Thunk-и. Но выделяеть экшены я не стал, они прям внутри thunk-ов, но как я
понимаю, сигнатура сохраняется как и в effector, по этому с thunk-ами будет работать точно так же.
...
Так же буду рад вопросам, критики, дополнениям итд.
